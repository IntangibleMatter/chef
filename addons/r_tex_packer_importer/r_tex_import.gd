...
#		options.image = source_file.replace(".json", ".png").replace(".xml", ".png").replace(".rtpa", ".png").replace(".rtpb", ".png")
	...
	match source_file.split(".")[-1]:
		...
		"rtpb":
			...
			print("rtpb")
		"json":
			tex = import_json(file, options, source_file)
		"xml":
			tex = import_xml(file, options, source_file)
	
	if not tex == {}:
		for tx in tex:
			var regex := RegEx.new()
			regex.compile("\\.(rtp(a|b)|json|xml)")
			var img_name := regex.sub(source_file.get_basename().replace(source_file.get_base_dir(), ""), "")
			var img_path :="{0}/{1}.{2}".format([source_file.get_base_dir() + "/" + img_name, tx, _get_save_extension()])
			if not DirAccess.dir_exists_absolute(source_file.get_base_dir() + "/" + img_name):
				DirAccess.make_dir_recursive_absolute(source_file.get_base_dir() + img_name)
...
func import_rtpa(file: FileAccess, opt: Dictionary, source_file: String) -> Dictionary:
	...
	if opt.image:
		img = load(opt.image)
#	else:
#		img = load(source_file.replace(".rtpa", ".png"))
	...
	for tex in items:
		...
		if spr[0] == "s":
			if spr_f[7] == 1: # trimmed
				atlas.region = Rect2i(spr_f[2], spr_f[3], spr_f[10] + spr_f[6], spr_f[11] + spr_f[6])
			else:
...
#	print(bytes)
	...
	for im in imgs:
		...
		if ints[7] == 1:
			atlas.region = Rect2i(ints[2], ints[3], ints[10] + ints[6], ints[11] + ints[6])
		else:
...
func import_json(file: FileAccess, opt: Dictionary, source_file: String) -> Dictionary:
	var data : Dictionary
	var img : Texture2D #= load(opt.image)
	if opt.image:
		img = load(opt.image)
...
#		img = load(source_file.replace(".rtpa", ".png").replace(".rtpb", ".png"))
	...
	if j_data.has("software"):
		if j_data.software.has("name"):
			if not j_data.software.name.begins_with("rTexPacker"):
				push_error("ERROR: File %s is not a valid rTexPacker atlas file" % source_file)
				return data
	if not j_data.has("sprites"):
		return data
	if not opt.image:
		if j_data.has("atlas"):
			if j_data.atlas.imagePath:
					img = load(source_file.get_base_dir() + "/" + j_data.atlas.imagePath)
	
	if not img:
		push_error("ERROR: No image found for file %s, aborting." % source_file)
		return data
	...
	for spr in j_data.sprites:
		...
		atlas.atlas = img
		if spr.trimmed:
			atlas.region = Rect2i(spr.position.x, spr.position.y, spr.trimRec.width + spr.padding, spr.trimRec.height + spr.padding)
		else:
...
func import_xml(file: FileAccess, opt: Dictionary, source_file: String) -> Dictionary:
	var data : Dictionary
	...
	if not opt.image:
		prints("xml", xml.get_attribute_name(0))
		print("asdasd")
		img = load(source_file.get_base_dir() + "/" + xml.get_named_attribute_value("imagePath"))
	
	print("DDFSASD")
	
	print(xml.get_node_type())
	while not (xml.get_node_type() == XMLParser.NODE_ELEMENT_END and xml.get_node_name() == "AtlasTexture"):
		xml.read()
